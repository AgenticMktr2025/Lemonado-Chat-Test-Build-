        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    ai_response = f"Error: Unable to connect to Ollama (Status: {response.status_code})"
        except Exception as e:
            logging.exception(f"Error connecting to Ollama: {e}")
            ai_response = "Sorry, I'm having trouble connecting to the AI model. Please make sure Ollama is running with the phi4:mini model."
